...
export default function BoardPage() {
  ...
  async function loadPosts() {
    const { data } = await supabase
      .from('posts')
      .select('*, profiles(name, department), applications(user_id)')
  ...
  async function apply(postId: string) {
    await supabase.from('applications').insert({ post_id: postId, user_id: userId })
    loadPosts()
  }
      ...
      <div className="max-w-2xl mx-auto px-4 py-6">
        ...
        <div className="space-y-3">
          ...
          {posts.map(post => {
            ...
            const applied = post.applications.some(a => a.user_id === userId)
            const full = post.applications.length >= post.slots - 1
            const remaining = post.slots - 1 - post.applications.length
            return (
              <div key={post.id} className={`bg-white rounded-2xl p-4 shadow-sm border ${isMine ? 'border-l-4 border-l-green-500' : 'border-gray-100'}`}>
                ...
                <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-50">
                  <span className="text-xs text-gray-400">
                    {post.applications.length > 0
                      ? `${post.applications.length}人が応募中`
                      : 'まだ応募なし'}
                  </span>
                  <div className="flex items-center gap-2">
                    <span className={`text-xs px-2 py-1 rounded-full ${full ? 'bg-gray-100 text-gray-400' : 'bg-green-50 text-green-600'}`}>
                      {full ? '満員' : `残り${remaining}人`}
                    </span>
                    {isMine ? (
                      <span className="text-xs text-green-600 font-medium">自分の募集</span>
                    ) : applied ? (
                      <button onClick={() => cancel(post.id)}
                        className="text-xs border rounded-lg px-3 py-1.5 text-gray-500 hover:bg-gray-50">キャンセル</button>
                    ) : full ? (
                      <button disabled className="text-xs border rounded-lg px-3 py-1.5 text-gray-300">満員</button>
                    ) : (
