...
export default function LoginPage() {
  ...
  async function handleSubmit() {
    ...
    if (mode === 'signup') {
      ...
      if (error) setMessage(error.message)
      else setMessage('パスワード再設定メールを送りました。メール内のリンクをクリックしてください。')
    }
      ...
      <div className="bg-white p-8 rounded-2xl shadow max-w-sm w-full">
        ...
        <div className="space-y-3">
          ...
          <button onClick={handleSubmit} disabled={loading || !email || (mode !== 'reset' && !password)}
            ...
            {loading ? '処理中...' : mode === 'signup' ? '新規登録' : mode === 'reset' ? '再設定メールを送る' : 'ログイン'}
          </button>
          {mode === 'login' && <>
            <button onClick={() => switchMode('signup')} className="w-full text-sm text-gray-500 hover:text-gray-700 text-center">
              初めての方はこちら（新規登録）
            </button>
            <button onClick={() => switchMode('reset')} className="w-full text-sm text-gray-400 hover:text-gray-600 text-center">
              パスワードをお忘れの方
            </button>
          </>}
          {mode !== 'login' && (
            <button onClick={() => switchMode('login')} className="w-full text-sm text-gray-500 hover:text-gray-700 text-center">
              ログインに戻る
            </button>
          )}
        </div>
